...
TRIC+ has one write opcode (`0x02 write_value`). The trailing `duration_ms` field decides the tier. The optional second opcode (`0x05 write_ttl`) covers the "I already wrote, now attach or extend a TTL" case.

```
write_value("user:42", "alice")          → SQLite (persistent, survives restart)  [opcode 0x02, duration_ms = 0]

write_value("session:abc", "token", 3600s)
                                         → BTreeMap, 1h TTL                       [opcode 0x02, duration_ms = 3600000]

write_value("cached:row", bytes)         → SQLite,
write_ttl("cached:row", 300s)            → promoted to BTreeMap with 300s TTL     [opcode 0x02 then 0x05]
```

The second form is the `SET k v EX t` primitive — one operation, one round-trip, no SQLite involvement.
...
### Supersession by transient write

When `write_value` carries a non-zero `duration_ms`, the entry is installed directly in BTreeMap with the given TTL. If a persistent entry for the same key already exists in SQLite, it is deleted — the new transient entry supersedes it. After the TTL expires, the key is gone from both tiers.

```
write_value("user:42", "alice", 0)             → SQLite
write_value("user:42", "alice-temp", 60_000)   → BTreeMap (60s), SQLite row deleted
